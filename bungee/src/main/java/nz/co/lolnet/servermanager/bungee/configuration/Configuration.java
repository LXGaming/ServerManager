...
public class Configuration implements IConfiguration {
    ...
    public void loadConfiguration() {
        ...
        ServerManager.getInstance().getLogger().info("Loaded configuration files.");
    }
    
    @Override
    public void saveConfiguration() {
        saveObject(getConfig(), "config.json");
        ServerManager.getInstance().getLogger().info("Saved configuration files.");
    }
    ...
    private Object loadObject(Object object, String name) {
        try {
            ...
            File file = ServerManager.getInstance().getPath().resolve(name).toFile();
            ...
            ServerManager.getInstance().getLogger().error("Encountered an error processing {}::loadObject", getClass().getSimpleName(), ex);
            return object;
        }
    }
    ...
    private boolean saveObject(Object object, String name) {
        try {
            ...
            if (parentFile != null && !parentFile.exists() && parentFile.mkdirs()) {
                ServerManager.getInstance().getLogger().info("Successfully created directory {}.", parentFile.getName());
            }
            
            if (!file.exists() && file.createNewFile()) {
                ServerManager.getInstance().getLogger().info("Successfully created file {}.", file.getName());
            }
            
            Files.write(file.toPath(), getGson(object).toJson(object, object.getClass()).getBytes(StandardCharsets.UTF_8));
            return true;
        } catch (IOException | OutOfMemoryError | RuntimeException ex) {
            ServerManager.getInstance().getLogger().error("Encountered an error processing {}::saveObject", getClass().getSimpleName(), ex);
            return false;
        }
    }
