...
public class StatusSessionHandler implements SessionHandler {
    ...
    public void activated() {
        ...
        ServerManager.getInstance().getEventManager().fireAndForget(new ConnectionEventImpl.Status(Platform.CLIENT, connection));
    }
    ...
    public boolean handle(StatusPacket packet) {
        Client.getInstance().getLogger().info(Toolbox.GSON.toJson(packet.getStatus()));
        return true;
    }
}