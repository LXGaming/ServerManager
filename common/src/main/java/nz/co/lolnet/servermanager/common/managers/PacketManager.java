...
package nz.co.lolnet.servermanager.common.managers;
...
public class PacketManager {
    ...
    public static boolean process(JsonObject jsonObject) {
        ...
        if (Toolbox.isBlank(packetId)) {
            AbstractServerManager.getInstance().getLogger().warn("Received invalid packet");
            return false;
        }
        
        Class<? extends AbstractPacket> packetClass = getPacket(packetId).orElse(null);
        if (packetClass == null) {
            AbstractServerManager.getInstance().getLogger().warn("Cannot find packet class {}", packetId);
            return false;
        }
        
        AbstractPacket packet = Toolbox.parseJson(jsonObject.get("data"), packetClass).orElse(null);
        if (packet == null) {
            AbstractServerManager.getInstance().getLogger().warn("Failed to deserialize packet {}", packetId);
            return false;
        }
        
        AbstractServerManager.getInstance().getLogger().debug("Processing {}: {}", packetClass.getSimpleName(), new Gson().toJson(packet));
        
        try {
            packet.process();
            return true;
        } catch (Exception ex) {
            AbstractServerManager.getInstance().getLogger().error("Encountered an error processing {}::process", "PacketManager", ex);
            return false;
        }
    }
    ...
    public static boolean registerPacket(Class<? extends AbstractPacket> packetClass) {
        if (getPacket(packetClass.getSimpleName()).isPresent()) {
            AbstractServerManager.getInstance().getLogger().warn("{} has already been registered", packetClass.getSimpleName());
            return false;
        }
